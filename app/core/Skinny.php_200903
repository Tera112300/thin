...
// テンプレート関連
$skConf['TEMPLATE']['BASEDIR'] = __DIR__ . '/../../view/template';         // テンプレートを保存するベースディレクトリ（特に指定がなければ空文字）
...
$skConf['PLUGIN']['DIR']      = __DIR__ . '/plugin'; // プラグインのPHPファイル置き場
...
class Skinny
{
	...
	public function _sfParseSkin($str, $type = null)
	{
		...
		switch ($com) {
			...
			case 'echo':
				return $this->_skTags_echo($prm);
				break;

			case 'dval':
				return $this->_skTags_dval($prm);
				break;
			case 'each':
				return $this->_skTags_each($prm);
				break;
			case 'for':
				return $this->_skTags_for($prm);
				break;

			case 'keach':
				return $this->_skTags_keach($prm);
				break;
			case 'kecho':
				return $this->_skTags_kecho($prm);
				break;

			case 'if':
				return $this->_skTags_if($prm);
				break;
			case 'elseif':
				return $this->_skTags_elseif($prm);
				break;

			case 'ifs':
				return $this->_skTags_ifs($prm);
				break;
			case 'elseifs':
				return $this->_skTags_elseifs($prm);
				break;

			case 'andif':
				return $this->_skTags_andif($prm);
				break;
			case 'andifs':
				return $this->_skTags_andifs($prm);
				break;

			case 'orif':
				return $this->_skTags_orif($prm);
				break;
			case 'orifs':
				return $this->_skTags_orifs($prm);
				break;

			case 'def':
			case 'ifdef':
			case 'defined':
				return $this->_skTags_def($prm);
				break;

			case 'even':
			case 'ifeven':
				return $this->_skTags_ifeven($prm);
				break;

			case 'var':
			case 'variable':
				return $this->_skTags_var($prm);
				break;

			case 'calc':
			case 'vcalc':
			case 'varcalc':
				return $this->_skTags_calc($prm);
				break;

			case 'replace':
				return $this->_skTags_replace($prm);
				break;

			case 'plg':
			case 'plugin':
				return $this->_skTags_plugin($prm);
				break;

			case 'elseeven':	//これは将来的に仕様が変わるか無くすかも
			case 'elsedef':		//これは将来的に仕様が変わるかも
			case 'else':
				return $this->_skTags_else();
				break;

			case 'external':
				if ($this->skConf['EXTERNAL_CALLBACK']) return $this->_skTags_external($prm);
				break;
...
*/
	...
	{
		$code = $this->_skReplacedCode(__DIR__ . "/../../view/" . $tpl, $param, $tplcode);
		@ob_end_clean();
		@ob_start();
		eval("?>" . $code);
